...
class Metadata:
	...
	def __init__(self, obj_in, input_type='blank'):
		self.obj_in = obj_in
		...
		#obj_in is a string
			...
			raise Exception('Input type does not exist')
		...
		try:
			if extension_check:
				if(str(splitext(file)[1]) != '.tgml'):
					raise Exception('File is not a .tgml file')
			if tag_check:
				if(etree.tostring(self.element.tag) != 'Tgml'):
					raise Exception('TGML file does not contain a <Tgml> or <Tgml/> tag at the beginning / end of the file')
			return 1
		except Exception as err:
			print(err)
			return 0
		...
		if children_check:
			for child in self.element.xpath('./*'):
				if child.tag not in self.SUPPORTED_CHILDREN:
					raise Exception('Element does not support this child: ' + str(child.tag))